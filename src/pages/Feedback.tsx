...
export default function Feedback() {
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', tour: '', rating: 0, feedback: '',
  });
          ...
          <div className="max-w-2xl mx-auto">
            ...
            >
              <div className="flex items-center gap-3 mb-6 sm:mb-8">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl font-serif font-bold text-foreground no-select">Leave Feedback</h2>
                  <p className="text-xs sm:text-sm text-muted-foreground">Tell us about your experience</p>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Which tour did you take?</label>
                  <select
                    value={formData.tour}
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-3">How would you rate your experience? *</label>
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star className={`w-8 h-8 sm:w-10 sm:h-10 transition-colors ${star <= (hoveredRating || formData.rating) ? 'fill-accent text-accent' : 'text-muted-foreground'}`} />
                      </button>
                    ))}
                  </div>
                  {formData.rating > 0 && (
